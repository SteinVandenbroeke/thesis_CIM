...
def run_inference(
        ...
        check_expected_results=False,flag=None):
    
    
    parent_func, child_func = get_eval_functions()
    is_parent = ind_range is None   # True

    def result_getter():
        if is_parent: # test_net_on_dataset
            # Parent case:
            ...
            for i in range(len(cfg.TEST.DATASETS)):
                ...
                mat_file = None # cfg.TEST.REFINE_FILES[i] # None #
                # print(proposal_file)
                ...
                results = parent_func(  # test_net_on_dataset
                    ...
                    mat_file,
                    output_dir,
                    multi_gpu=multi_gpu_testing,flag=flag
                )
...
def test_net_on_dataset(
    ...
    if multi_gpu: # False
        ...
        all_boxes = test_net(
            args, dataset_name, proposal_file, mat_file,  output_dir, gpu_id=gpu_id,flag=flag
        )
    test_timer.toc()
...
def test_net(
    ...
    roidb, dataset, start_ind, end_ind, total_num_images = get_roidb_and_dataset(
        dataset_name, proposal_file, mat_file, ind_range
    )
    ...
    for i, entry in enumerate(roidb):
        ...
        masks = entry['masks']
        mat = entry['mat'] # np.empty((1, 21), dtype=np.float32) # entry['mat']
        path = entry['image']
    ...
    cfg_yaml = yaml.dump(cfg)
    if 'train' in dataset_name: # False:#
...
def get_roidb_and_dataset(dataset_name, proposal_file, mat_file, ind_range):
    """Get the roidb for the dataset specified in the global cfg. Optionally
    restrict it to a range of indices if ind_range is ainference pair of integers.
    """
    dataset = JsonDataset(dataset_name)
    if cfg.TEST.PRECOMPUTED_PROPOSALS: # True
