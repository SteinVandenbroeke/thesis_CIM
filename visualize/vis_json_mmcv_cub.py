...
def gt_dataset_mask(cocoGT, save_dir, name_mapping):
    ...
    for index in tqdm(range(len(imgIds)), total=len(imgIds)):
        ...
        # Ensure directories exist if the path has subfolders
        ...
        try:
            ...
            plt.axis("off")
            plt.savefig(full_save_path, dpi=300, bbox_inches='tight', pad_inches=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--result_file_path", type=str, required=False, default="")
    ...
    if "gt" not in parser.dataset:
        ...
        try:
            if "annotations" in res.keys():
                temp_filename = os.path.join("./data/trash", './temp.json')
        ...
        # Worker lowered from 12 to 4 to protect your RAM
        worker = 4
        ...
        for worker_id in range(worker):
            if worker_id + 1 != worker:
                p = multiprocessing.Process(target=dataset_mask,
                                            args=(cocoGt, cocoDt1, save_dir,
                                                  imgIds[worker_id * per_len:(worker_id + 1) * per_len], parser.num,
                                                  thr, cls_name_map))
            else:
                p = multiprocessing.Process(target=dataset_mask,
                                            args=(
                                            cocoGt, cocoDt1, save_dir, imgIds[worker_id * per_len:], parser.num, thr,
                                            cls_name_map))
            jobs.append(p)
            p.start()
    ...
    # --- NEW CUB GT MODES ---
    elif parser.dataset == "cub_val_gt":
        label_file = "./data/CUB_200_2011/CUB_as_COCO/annotations/instances_val2017.json"
        root = "./data/CUB_200_2011/images"
        save_dir = parser.save_dir
        os.makedirs(save_dir, exist_ok=True)
    ...
    elif parser.dataset == "cub_train_gt":
        label_file = "./data/CUB_200_2011/CUB_as_COCO/annotations/instances_train2017.json"
        root = "./data/CUB_200_2011/images"
        save_dir = parser.save_dir
        os.makedirs(save_dir, exist_ok=True)
