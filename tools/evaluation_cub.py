...
def parse_args():
    ...
    # ADDED "cubval" TO CHOICES
    parser.add_argument(
        '--dataset', required=True, choices=["coco2017val", "coco2017test", "voc2012sbdval", "cubval"],
    ...
    parser.add_argument(
        ...
        default=[], nargs='*')
    # Lowered default worker count to 12 for stability during heavy matrix processing
    parser.add_argument(
        '--total_process', dest='total_process', default=12, type=float,
...
def eval(name, total, roidb):
    ...
    for i, entry in enumerate(roidb):
        ...
        # --- CUB DATASET MAT LOADING ---
        if "cub" in args.dataset:
            cob_original_file = './data/coco2017/COB-COCO'
            file_name = os.path.basename(entry['image']).replace('.jpg', '.mat')

            try:
                COB_proposals = scipy.io.loadmat(
                    os.path.join(cob_original_file, file_name),
                    verify_compressed_data_integrity=False)['maskmat']
                # Correctly unpack the 1D object array created by GrabCut
                mask_proposals = np.array([np.array(p) for p in COB_proposals])
            except Exception as e:
                print(f"Warning: Failed to load {file_name}. Skipping masks.")
                mask_proposals = []
            cls_num = 201
        ...
        for cls_idx in range(1, cls_num):
            for instance_idx in range(len(cls_boxes[cls_idx])):
                COB_ind = cls_inds[cls_idx][instance_idx]

                # Safety check if proposals failed to load
                if len(mask_proposals) == 0 or COB_ind >= len(mask_proposals):
                    continue

                mask = mask_proposals[COB_ind]

                # Process for both VOC (21) and CUB (201)
                if cls_num == 21 or cls_num == 201:
    ...
    with open(args.result_path[:-14] + 'sbd_instance_pred_origin' + '_' + str(range_begin) + '_' + str(
            range_end) + '.json', 'w') as f:
...
if __name__ == '__main__':
    ...
    # --- CUB DATASET REGISTRATION ---
    if args.dataset == "cubval":
        cfg.TEST.DATASETS = ('coco_2017_val',)  # Piggyback on COCO data engine
        cfg.MODEL.NUM_CLASSES = 200
        annFile = "./data/CUB_200_2011/CUB_as_COCO/annotations/instances_val2017.json"

    elif args.dataset == "coco2017val":
    ...
    total_process = int(args.total_process)
