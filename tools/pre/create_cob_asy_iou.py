...
# --- PURE PYTORCH ASYMMETRIC IOU IMPLEMENTATION ---
def mask_asymmetric_iou(masks_a, masks_b):
    """
    Calculates Asymmetric Intersection over Union natively in PyTorch on the GPU.
    Asymmetric IoU = Intersection(A, B) / Area(A)
    This measures how much of mask A is swallowed by mask B.
    """
...
def generate_cub_as_coco(imgIds, cocoGt, data_dir):
    ...
    for index in tqdm(range(len(imgIds))):
        ...
        # Safely parse the filename without string slicing
        ...
        # Move array to PyTorch GPU
        mask_proposals_pt = torch.tensor(mask_proposals, device=device, dtype=torch.bool)

        iou_map = []
        for j in range(num_proposal):
            proposal_iou = mask_asymmetric_iou(mask_proposals_pt, mask_proposals_pt[j].unsqueeze(0))
...
if __name__ == '__main__':
    ...
    if dataset == "cub_as_coco":
        ...
        data_dir = "./data/cob_asy_iou/coco2017"
    else:
        # Halt others to keep code perfectly clean for CUB
